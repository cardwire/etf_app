...
# plot the dividends of SPY in plotly as line chart^
fig = px.bar(dividends, x=dividends.index, y=dividends, title=f'{symbol} Dividends')
#change background color to white, add gridlines in grey, and change font size
fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_size=12, yaxis=dict(gridcolor='lightgrey'))
fig.update_traces(color = "orange")
#center title
fig.update_layout(title_x=0.5)
fig.show()
...
sectors = ticker.get_funds_data().sector_weightings

# Convert the dictionary to a DataFrame
sectors_df = pd.DataFrame(list(sectors.items()), columns=['sector', 'weight'])

# plot the sector weightings of SPY in plotly as bar chart
fig = px.bar(sectors_df, x='sector', y='weight', title=f'{symbol} Sector Weightings')
#change background color to white, add gridlines in grey, and change font size
fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_size=12, yaxis=dict(gridcolor='lightgrey'))
#set bar outlines to black
fig.update_traces(marker_line_color='black', marker_line_width=1)
#change order of sectors to descending
...
symbol = 'SPY'
ticker = yf.Ticker(f'{symbol}')
assetclasses = ticker.get_funds_data().asset_classes

# Convert the dictionary to a DataFrame
assetclasses_df = pd.DataFrame(list(assetclasses.items()), columns=['asset_class', 'weight'])

# plot the asset classes of SPY in plotly as bar chart
fig = px.bar(assetclasses_df, x='asset_class', y='weight', title=f'{symbol} Asset Classes')
#change background color to white, add gridlines in grey, and change font size
fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_size=12, yaxis=dict(gridcolor='lightgrey'))
#set bar outlines to black
fig.update_traces(marker_line_color='black', marker_line_width=1)
#change order of asset classes to descending
fig.update_xaxes(categoryorder='total descending')
#set bar colors to deepskyblueblue
fig.update_traces(marker_color='lightseagreen')
#center title
...
symbol = 'SPY'
ticker = yf.Ticker(f'{symbol}')
holdings = ticker.get_funds_data().top_holdings

# Convert the dictionary to a DataFrame
holdings_df = pd.DataFrame(holdings)

# plot the top holdings of SPY in plotly as bar chart
fig = px.bar(holdings_df, x='Name', y='Holding Percent', title=f'{symbol} Top Holdings')
#change background color to white, add gridlines in grey, and change font size
fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_size=12, yaxis=dict(gridcolor='lightgrey'))
#set bar outlines to black
fig.update_traces(marker_line_color='black', marker_line_width=1)
#change order of holdings to descending
fig.update_xaxes(categoryorder='total descending')
#set bar colors to deepskyblueblue
fig.update_traces(marker_color='lightseagreen')
#center title
